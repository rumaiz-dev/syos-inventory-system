...
public class AddProductCommand implements Command {
	...
	public void execute() {
		...
		String code = getProductCodeInput();
		String name = getProductNameInput();
		double price = getProductPriceInput();
	...
	private String getProductCodeInput() {
		...
		while (true) {
			...
			} else if (code.length() < CommonVariables.MIN_CODE_LENGTH
					|| code.length() > CommonVariables.MAX_CODE_LENGTH) {
				System.out.printf("Error: Product code must be between %d and %d characters long.%n",
						CommonVariables.MIN_CODE_LENGTH, CommonVariables.MAX_CODE_LENGTH);
	...
	private String getProductNameInput() {
		...
		while (true) {
			...
			} else if (name.length() < CommonVariables.MIN_NAME_LENGTH
					|| name.length() > CommonVariables.MAX_NAME_LENGTH) {
				System.out.printf("Error: Product name must be between %d and %d characters long.%n",
						CommonVariables.MIN_NAME_LENGTH, CommonVariables.MAX_NAME_LENGTH);
			} else {
				return name;
			}
		}
		...
		while (true) {
			...
			try {
				...
				if (price < CommonVariables.MIN_PRICE) {
					System.out.printf("Error: Price cannot be negative. Minimum price is %.2f.%n",
							CommonVariables.MIN_PRICE);
				} else {
					return price;
				}
